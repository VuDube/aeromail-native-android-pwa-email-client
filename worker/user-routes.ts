...
export function userRoutes(app: Hono<{ Bindings: Env }>) {
  ...
  app.get('/api/auth/callback', async (c) => {
    ...
    if (!code) return bad(c, "AUTH_CODE_MISSING");
    try {
  ...
  app.get('/api/emails', async (c) => {
    ...
    let params: any[] = [];
    if (searchQuery) {
      query = `SELECT t.*, (SELECT GROUP_CONCAT(from_name, ', ') FROM (SELECT DISTINCT from_name FROM emails WHERE thread_id = t.id)) as participantNames FROM threads t WHERE (subject LIKE ? OR snippet LIKE ?) ORDER BY last_message_at DESC`;
